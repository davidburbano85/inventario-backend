...
public class EmpresaController : ControllerBase
{
    ...
    public async Task<IActionResult> ObtenerEmpresaPorUsuario()
    {
    ...
    public async Task<IActionResult> ObtenerEmpresaActual()
    {
        var empresaIdClaim = User.FindFirst("EmpresaId")?.Value;

        if (string.IsNullOrEmpty(empresaIdClaim))
            return BadRequest("El usuario no tiene empresa activa.");

        if (!Guid.TryParse(empresaIdClaim, out var empresaId))
            return BadRequest("EmpresaId inválido en el token.");
    ...
    public async Task<IActionResult> ActualizarEmpresa([FromBody] EmpresaDTO dto)
    {
        ...
        var usuarioId = Guid.Parse(User.FindFirst(ClaimTypes.NameIdentifier)?.Value);
        // Toma el id del usuario desde el token (ClaimTypes.NameIdentifier) y lo
        // convierte a Guid para usarlo en tu sistema.
       // Si no existe o no es Guid válido, lanza excepción.


        Console.WriteLine($"ususarioId controller: {usuarioId}");
        var result = await _servicio.ActualizarEmpresaAsync(usuarioId, dto);
        return Ok(result);
    }

    [HttpDelete("{empresaId:guid}")]//ruta {} 
    public async Task<IActionResult> EliminarEmpresa( Guid empresaId)
    {
